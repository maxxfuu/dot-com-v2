...
Now purely from the arithmetic side of things, there are 2 memory reads for every fused multiply add (FMA), precisely `2K = 8192` reads per output element. This is the same number that the previous kernel achieved prior to the shared memory tiling optimization, meaning we are still reading the same amount of data just from a different memory. So instead of reading from the slow global memory that takes 500 cycles to load, we are doing the loading from the shared memory, which takes 25 cycles to load. 
