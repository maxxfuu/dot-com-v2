...
Kernel 3 runs in **47.007 ms at 2923.8 GFLOP/s**, against kernel 2's 47.213 ms, the same 8.1% of cuBLAS at `M = N = K = 4096`, and the 0.4% difference is inside the run-to-run spread of the two kernels. It is a no-op, which is the point: the mapping is free, and now it is written down.

What it does not fix is the count. Every element of A is still loaded once for every thread in that column of C, and every element of B once for every thread in that row. It is the same value pulled from global memory over and over by threads sitting in the same block, which could have shared it.
